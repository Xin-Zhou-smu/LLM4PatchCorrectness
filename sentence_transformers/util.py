...
def cos_sim(a: Tensor, b: Tensor, threshold=0.9):
    """
    Computes the cosine similarity cos_sim(a[i], b[j]) for all i and j.
    :return: Matrix with res[i][j]  = cos_sim(a[i], b[j])
    ...
    results = torch.mm(a_norm, b_norm.transpose(0, 1))
    

    zero = torch.zeros_like(results)
    results = torch.where(results < threshold, zero, results)
